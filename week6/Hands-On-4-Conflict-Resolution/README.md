...
- `terminal-output.txt` — the real captured terminal output for the whole flow: `hello.xml` committed with different content on both `GitWork` and `master`, the actual `CONFLICT (add/add)` message from `git merge`, the raw `<<<<<<< HEAD` / `=======` / `>>>>>>> GitWork` markers as Git really produced them, then the manual resolution and final clean merge commit.
